...
from network import Net, NetCCFFF, one_one_Net, one_fl_Net, one_fl_Net_con_nomal

from torch.utils.tensorboard import SummaryWriter
# default `log_dir` is "runs" - we'll be more specific here
writer = SummaryWriter('runs/lr00005_64_one_fl_conv_batch')
...
# net = one_fl_Net(input_channels)

net = one_fl_Net_con_nomal(input_channels)
print(net)
...
with open('accuracy_stats.txt', 'w') as f:
    for a in accuracies:
        f.write("%s\n" % str(a))
with open('learningRate.txt', 'w') as f:
    for b in lr_:
        f.write("%s\n" % str(b))
