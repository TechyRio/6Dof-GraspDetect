...
GraspDetector::GraspDetector(const std::string &config_filename) {                          //一旦创建对象成功，就会直接输出相应的参数配置。这一块都是读取配置文件的。
  Eigen::initParallel();
...
std::vector<std::unique_ptr<candidate::Hand>> GraspDetector::detectGrasps(
  ...
  double t0_total = omp_get_wtime();  //并行处理时获取程序用的时间。
...
GraspDetector::filterGraspsWorkspace(
  ...
  for (int i = 0; i < hand_set_list.size(); i++) {
    ...
    for (int j = 0; j < hands.size(); j++) {
      ...
      Eigen::Vector3d left_bottom =//表示一个坐标。left_bottom（0）为x，left_bottom（1）为y   。。。
      ...
      Eigen::Vector3d right_top =
          right_bottom + hand_geometry.depth_ * hands[j]->getApproach();//他好像写错了，两个为啥一样呢。不应该一样的。这里我把left_bottom改成了ight_bottom
      ...
      // Ensure the object fits into the hand and avoid grasps outside the       确保抓手不与物体碰撞，同时在工作空间内。x.minCoeff()表示返回矩阵中的最小值。
      // workspace.
      if (hands[j]->getGraspWidth() >= min_aperture_ &&   //限定抓手宽度。
          hands[j]->getGraspWidth() <= max_aperture_ &&
          x.minCoeff() >= workspace[0] && x.maxCoeff() <= workspace[1] &&//那四个点之间的x,y,z坐标值是否在限定的坐标范围内。
