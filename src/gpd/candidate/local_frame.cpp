...
void LocalFrame::print() {
  std::cout << "sample: " << sample_.transpose() << std::endl;            //竟然可以转置后输出流。。
...
void LocalFrame::findAverageNormalAxis(const Eigen::MatrixXd &normals) {
  ...
  Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> eigen_solver(                 //求特征值和特征向量。
  ...
  int min_index;                                                                                //最小值的索引，计算矩阵中的最大值和最小值用   ，若是想要得到位置，需要给定一个值。这里为min_index。
