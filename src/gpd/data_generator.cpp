...
void DataGenerator::generateData() {
  ...
  int num_train_views =1;
  const int n_train = store_step * num_train_views * max_grasps_per_view_;

  const int n_test =  store_step * num_train_views * max_grasps_per_view_;
  ...
  util::Plot plotter(1, 8);
   ...
   //################################################
    ...
    util::Cloud mesh =  loadpcd(prefix + "_gt.pcd") ;
...
//##############3################改这里了############## 对应头文件也改了
    ...
    for (int j = 0; j < num_views_per_object_; j++) {
      ...
      //                   view_points);
         ...
         util::Cloud cloud( data_root_+ std::to_string(i) + "_gt.pcd",
                        view_points);
      if (remove_nans_) {
        cloud.removeNans();
      }
      ...
      int count_num = 1;
      while ((positives_view.size() < min_grasps_per_view_ )  &&  (count_num<2)) {
...
//##############3################改这里了############## 对应头文件也改了
    ...
    for (int j = 0; j < num_views_per_object_; j++) {
      ...
      //                   view_points);
         ...
         util::Cloud cloud( data_root_+ std::to_string(j + 1) + "_gt.pcd",
                        view_points);
      if (remove_nans_) {
        cloud.removeNans();
      }
...
util::Cloud DataGenerator::loadpcd(const std::string &mesh_file_path){
